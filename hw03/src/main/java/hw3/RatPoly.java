...
//        throw new RuntimeException("RatPoly.degree() is not yet implemented");    	
    ...
    public RatNum getCoeff(int pow) {
    	if (pow >= 0 && pow <= degree && !this.equals(RatPoly.ZERO)) return this.coeffs[pow]/*this.coeffs[pow]*/;
    	else return RatNum.ZERO;
    	// TODO: Fill in this method, then remove the RuntimeException
//        throw new RuntimeException("RatPoly.getCoeff() is not yet implemented");
    ...
    public boolean isNaN() {
    	...
    	for (int i = 0; i <= degree; i++) {
    		if (this.coeffs[i].isNaN()) return true;
    	}
    	return false;
    	// TODO: Fill in this method, then remove the RuntimeException
...
//        throw new RuntimeException("RatPoly.scaleCoeff() is not yet implemented");
    ...
    public RatPoly negate() {
    	if (this.equals(RatPoly.ZERO) ) return RatPoly.ZERO;
    	RatNum[] ret = new RatNum[degree+1];
    	for (int i = 0; i <= degree; i++) {
    		ret[i] = RatNum.ZERO.sub(this.coeffs[i]);
    	}
...
//        throw new RuntimeException("RatPoly->negate() is not yet implemented");
    ...
    public RatPoly add(RatPoly p) {
    	if (this.equals(RatPoly.NaN) || p.equals(RatPoly.NaN) ) return RatPoly.NaN;
    	if (this.equals(p.negate())) return RatPoly.ZERO;
    	RatNum[] ret = new RatNum[Math.max(this.degree(), p.degree()) + 1];
    	for (int i = 0; i <= Math.max(this.degree(), p.degree()); i++) {
			ret[i] = this.getCoeff(i).add(p.getCoeff(i));
		}
    	ret = RatPoly.removeLeadingZeros(ret);
    	return new RatPoly(ret);
        // TODO: Fill in this method, then remove the RuntimeException
//        throw new RuntimeException("RatPoly.add() is not yet implemented");
    ...
    public RatPoly sub(RatPoly p) {
    	if (this.equals(RatPoly.NaN) || p.equals(RatPoly.NaN) ) return RatPoly.NaN;
    	if (this.equals(p) ) return RatPoly.ZERO;
    	RatNum[] ret = new RatNum[Math.max(this.degree(), p.degree()) + 1];
    	int i = 0;
    	for (; i <= Math.max(this.degree(), p.degree()); i++) {
    		ret[i] = this.getCoeff(i).sub(p.getCoeff(i));
    	}
    	ret = RatPoly.removeLeadingZeros(ret);
    	return new RatPoly(ret);
        // TODO: Fill in this method, then remove the RuntimeException
//        throw new RuntimeException("RatPoly->sub() is not yet implemented");
    ...
    public RatPoly mul(RatPoly p) {
    	if (this.equals(RatPoly.NaN) || p.equals(RatPoly.NaN) ) return RatPoly.NaN;
    	if (this.equals(RatPoly.ZERO) || p.equals(RatPoly.ZERO) ) return RatPoly.ZERO;
    	RatNum[] temp = null;
    	RatNum[] ret = new RatNum[this.degree() + p.degree() + 1];
    	for (int i = 0; i < ret.length; i++) {
    		ret[i] = RatNum.ZERO;
    	}
    	for (int i = 0; i <= this.degree(); i++) {
    		temp = RatPoly.shiftExp(p.coeffs, i);	// temp = copy of p.coeffs, with exp's shifted
    		RatPoly.scaleCoeff(temp, this.getCoeff(i)); // apply scalar this.coeffs[i] to temp
        	for (int j = 0; j < Math.min(ret.length, temp.length); j++) {		// add to ret
        		if (!ret[j].equals(RatNum.ZERO) && !temp[j].equals(RatNum.ZERO)) ret[j] = ret[j].add(temp[j]);
        		else if (ret[j].equals(RatNum.ZERO) ) ret[j] = temp[j];
        	}
    	}
    	return new RatPoly(ret);
        // TODO: Fill in this method, then remove the RuntimeException
//        throw new RuntimeException("RatPoly->mul() is not yet implemented");
    ...
    public RatPoly div(RatPoly p) {
    	if (p.equals(RatPoly.ZERO)) return RatPoly.NaN;
    	else if (this.equals(RatPoly.ZERO)) return RatPoly.ZERO;
    	else if (p.equals(RatPoly.NaN) || this.equals(RatPoly.NaN) ) return RatPoly.NaN;
    	RatPoly q = RatPoly.ZERO;
    	RatPoly r = new RatPoly(this.coeffs);
    	RatPoly t = null;
    	int t_deg = 0;
    	RatNum t_leadCoeff = null;
    	RatNum r_leadCoeff = null;
    	RatNum p_leadCoeff = p.getCoeff(p.degree());
    	
    	while ( !r.equals(RatPoly.ZERO) && r.degree() >= p.degree()) {
    		
    		r_leadCoeff = r.getCoeff(r.degree());
    		t_leadCoeff = r_leadCoeff.div( p_leadCoeff );
    		
    		t_deg = r.degree() - p.degree();
    		t = new RatPoly(t_leadCoeff, t_deg);
...
//    	
//    	assert q.degree() == this.degree() - p.degree() + 1;
    	return q;
        // TODO: Fill in this method, then remove the RuntimeException
//        throw new RuntimeException("RatPoly->div() is not yet implemented");
    ...
    public double eval(double d) {
    	if (this.equals(RatPoly.ZERO)) return 0.0;
    	if (this.equals(RatPoly.NaN)) return Double.NaN;
    	if (this.degree == 0) return this.coeffs[degree].doubleValue();

    	Double ret = d*this.coeffs[degree].doubleValue() + this.coeffs[degree-1].doubleValue();
    	for (int i = this.degree() - 1; i > 0; i--) {
    		if (this.coeffs[i-1].isNaN()) return Double.NaN;
    		ret = d*ret + this.coeffs[i-1].doubleValue();
    	}
    	return ret;
        // TODO: Fill in this method, then remove the RuntimeException
//        throw new RuntimeException("RatPoly.eval() is not yet implemented");
    ...
    public RatPoly differentiate() {
    	...
    	if (this.coeffs.length == 0) return RatPoly.ZERO;
    	if (this.degree == 0) return RatPoly.ZERO;
    	RatNum[] ret = new RatNum[this.degree];
    	for (int i = 0; i < this.degree(); i++) {
    		ret[i] = this.getCoeff(i+1).mul(new RatNum(i+1));
    	}
...
//        throw new RuntimeException("RatPoly.differentiate() is not yet implemented");
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
    	if (this.isNaN()) return RatPoly.NaN;
    	if (this.equals(RatPoly.ZERO) && integrationConstant.equals(RatNum.ZERO)) return RatPoly.ZERO;
    	RatNum[] ret = null;
    	if (this.coeffs.length == 0) ret = new RatNum[1];
    	else ret = new RatNum[this.degree + 2];
    	
    	ret[0] = integrationConstant;
    	for (int i = 1; i < ret.length; i++) {
    		ret[i] = this.getCoeff(i-1).div(new RatNum(i));
    	}
...
//        throw new RuntimeException("RatPoly->antiDifferentiate() unimplemented!");
    ...
    public double integrate(double lowerBound, double upperBound) {
    	...
    	double F_b = this.antiDifferentiate(RatNum.ZERO).eval(upperBound);
    	double F_a = this.antiDifferentiate(RatNum.ZERO).eval(lowerBound);
    	return F_b - F_a;
        // TODO: Fill in this method, then remove the RuntimeException
//        throw new RuntimeException("RatPoly.integrate() is not yet implemented");
    ...
    // vvvvvvvvvvvvvvvvvv NEW PRIVATE HELPER FUNCTIONS vvvvvvvvvvvvvvvvvv

    
    /**
     * @param c The constant (RatNum) in the term which the new RatPoly equals.
     * @param e The exponent in the term which the new RatPoly equals.
     * @requires e >= 0
     * @effects Constructs a new RatPoly equal to "c*x^e". If c is zero, constructs
     *          a "0" polynomial.
     */
    private RatPoly(RatNum c, int e) {
    ...
    /**
     * Creates RatNum array where index of all items are increased by e to represent
     * 		   multiplying polynomial by x^e
     * 
     * @param arr The RatPoly from which to copy values from
     * @requires arr != null, e >= 0
     * @return a RatNum[] where index of all items are increased by e
     */
    private static RatNum[] shiftExp(RatNum[] arr, int e) {	
    	RatNum[] ret = new RatNum[arr.length + e];
    	for (int i = 0; i < e; i++) {
    		ret[i] = RatNum.ZERO;
    	}
    	for (int i = 0; i < arr.length; i++) {
    ...
    /**
     * Creates RatNum array that does not throw RuntimeException "coeffs[degree] is 0"
     * when it is used as argument for RatPoly constructor
     * 
     * @param arr The RatPoly from which to copy values from
     * @requires arr != null
     * @return a RatNum[] that is an exact copy of arr's values but resized so that
     * 		   coefficient for leading non-zero term is placed at highest index to
     * 		   represent the subsequent polynomial's degree
     */
    private static RatNum[] removeLeadingZeros(RatNum[] arr) {
    	int a = arr.length - 1;
    	for (; a >= 0; a--) {
    		if (!arr[a].equals(RatNum.ZERO)) break;	// break when no more leading zeros
    	}
    	if ( a < arr.length - 1) { // if new size is smaller, create resized array, copy values
    		RatNum[] temp = new RatNum[a + 1];
        	for (int i = 0; i <= a; i++) {
        		temp[i] = arr[i];
        	}
        	return temp;
